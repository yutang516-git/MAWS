    ...
    def init_weight(self):
        for m in self.children():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
class SimpleSegFormerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.channel_reducers = nn.ModuleList([
            nn.Conv2d(input_channels, reduced_channels, 1) for _ in range(4)
        ])

        # 融合层
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(reduced_channels * 4, reduced_channels, 3, padding=1),
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(reduced_channels, reduced_channels // 2,3,padding= 1),
            nn.BatchNorm2d(reduced_channels // 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(reduced_channels // 2, num_classes, 1)
        )

    def forward(self, res1: torch.Tensor, res2: torch.Tensor,
                res3: torch.Tensor, res4: torch.Tensor) -> torch.Tensor:
        ...
        for i, feature in enumerate(features):
            # 通道降维
            reduced = self.channel_reducers[i](feature)

            # 上采样到H/4
            if i == 0:
                upsampled = reduced
            elif i == 1:
                upsampled = F.interpolate(reduced, scale_factor=2, mode='bilinear', align_corners=False)
            elif i == 2:
                upsampled = F.interpolate(reduced, scale_factor=4, mode='bilinear', align_corners=False)
            else:
                upsampled = F.interpolate(reduced, scale_factor=8, mode='bilinear', align_corners=False)
...
class M2fnet(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        args = cfg.parse_args()
        self.sam = sam_model_registry["vit_b"](args,checkpoint='./sam_vitb.pth')
        ...
        self.segformer = SimpleSegFormerDecoder(input_channels=256, reduced_channels=64, num_classes=8)
    ...
    def forward(self, x, y, mode='Train'):
        ...
        y = y.repeat(1, 3, 1, 1)
        deepx, deepy,expert_outputs_list = self.image_encoder(x, y)  # b*256*16*16
        heatmaps = []
        ...
        x = self.segformer(res1, res2, res3, res4)

        ## heatmap
        feature = expert_outputs_list[0]
        feature = feature[0]
        ...
        feature = expert_outputs_list[1]
        feature = feature[0]
        heatmap = feature.detach().cpu().numpy()
        heatmap = heatmap.transpose(2, 0, 1)
        heatmap = np.mean(heatmap, axis=0)
        heatmap2 = (heatmap - heatmap.min())/(heatmap.max()-heatmap.min())

        feature = expert_outputs_list[2]
        feature = feature[0]
        heatmap = feature.detach().cpu().numpy()
        heatmap = heatmap.transpose(2, 0, 1)
        heatmap = np.mean(heatmap, axis=0)
        ...
        feature = expert_outputs_list[3]
        feature = feature[0]
        heatmap = feature.detach().cpu().numpy()
        heatmap = heatmap.transpose(2, 0, 1)
        heatmap = np.mean(heatmap, axis=0)
        heatmap4 = (heatmap - heatmap.min()) / (heatmap.max() - heatmap.min())
        # well = torch.stack(expert_outputs_list)
        # print(well.shape)

        return x, heatmap1, heatmap2,heatmap3,heatmap4
