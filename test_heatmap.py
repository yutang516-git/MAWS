...
from M2fnet_heatmap import M2fnet as M2fnet
...
def test(net, test_ids, all=False, stride=WINDOW_SIZE[0], batch_size=1, window_size=WINDOW_SIZE):
    ...
    for img,ir, gt, gt_e in tqdm(zip(test_images, test_irs, test_labels, eroded_labels), total=len(test_ids), leave=False):
        ...
        for i, coords in enumerate(
            ...
            # Do the inference
            outs, heatmap1, heatmap2 = net(image_patches, ir_patches, mode='Test')
            ...
            plt.savefig('./tu/heat'+str(index)+'.pdf', dpi=1200)
            index += 1
...
if MODE == 'Train':
    ...
    if DATASET == 'MAWS':
        ...
        for p, id_ in zip(all_preds, test_ids):
            ...
            io.imsave('./tu/_{}_tile_{}.png'.format('base', id_), img)
        ...
        net.load_state_dict(torch.load('./resultsh/UNetformer_epoch20_0.4849040569550816'), strict=False)
        net.eval()
        MIoU, all_preds, all_gts = test(net, test_ids, all=True, stride=32)
        print("MIoU: ", MIoU)
        for p, id_ in zip(all_preds, test_ids):
            img = convert_to_color(p)
            io.imsave('./results_heatmap/heat_{}_tile_{}.png'.format('base', id_), img)
